...
def make_bot(broker, clock, **overrides):
    ...
    defaults = dict(
        ...
        on_round_trip=lambda rt: round_trips.append(rt), now_fn=clock,
    )
    defaults.update(overrides)
    return SpreadCaptureBot(**defaults), round_trips


def test_stays_idle_when_spread_too_narrow():
    broker = FakeBroker(bid=1.00, ask=1.10)  # spread 0.10 < min 0.20
    bot, _ = make_bot(broker, FakeClock())
    bot.tick()
...
def test_skips_entry_if_improved_price_would_cross_ask():
    ...
    bot, _ = make_bot(broker, FakeClock(), min_spread_to_enter=0.0, num_steps=10)
    bot.tick()
...
def test_buy_timeout_with_zero_fill_cancels_and_returns_idle():
    broker = FakeBroker(bid=1.00, ask=2.00)
    clock = FakeClock()
    bot, _ = make_bot(broker, clock, buy_timeout_seconds=10)
    bot.tick()
...
def test_buy_timeout_with_partial_fill_hedges_filled_qty():
    broker = FakeBroker(bid=1.00, ask=2.00)
    clock = FakeClock()
    bot, _ = make_bot(broker, clock, qty=5, buy_timeout_seconds=10)
    bot.tick()
    buy_id = bot._buy_order_id
...
def test_sell_fill_logs_round_trip_with_correct_edge():
    broker = FakeBroker(bid=1.00, ask=2.00)
    clock = FakeClock()
    bot, round_trips = make_bot(broker, clock)
    bot.tick()
    broker.fill(bot._buy_order_id, dealt_qty=1, price=1.05)
...
def test_sell_reprices_toward_bid_after_timeout():
    broker = FakeBroker(bid=1.00, ask=2.00)
    clock = FakeClock()
    bot, _ = make_bot(broker, clock, sell_reprice_after_seconds=30, sell_reprice_step_ticks=2)
    bot.tick()
    broker.fill(bot._buy_order_id, dealt_qty=1, price=1.05)
